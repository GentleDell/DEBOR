...
def crop(img, center, scale, res, rot=0):
    ...
    new_y = max(0, -ul[0]), min(br[0], len(img)) - ul[0]
    new_x = max(0, -ul[1]), min(br[1], len(img[0])) - ul[1]
...
def uncrop(img, center, scale, orig_shape, rot=0, is_rgb=True):
    ...
    raise ValueError # check the datatype first
    img = np.array(Image.fromarray(img).resize(crop_shape, resample=Image.NEAREST))
